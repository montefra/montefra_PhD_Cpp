...
Likelihood::Likelihood(int dataset, ParseIni &ini, Theory &theory):
  this->dataset=dataset, this->ini=ini, this->theory=theory
{
  //read the dataset
  std::string pk_dataset = std::string("pk_dataset")+to_string(this->dataset);
  data = new Read_pk_files(pk_dataset);
...
void Likelihood::retrieve_paramnames(){
  ...
  for(size_t i=0; i<paramnames.size(); ++i){
    std::string temp = paramnames[i]+to_string(dataset); //add the dataset number
