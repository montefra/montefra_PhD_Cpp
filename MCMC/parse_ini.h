...
class ParseIni
{
  ...
  public:
     ...
     *==========================================================================*/
    template <class T>  //set template name
    int get_param( std::string parname, T *value );
    /*==========================================================================
     * Specialisation of a Template function
     * Read a single parameter following a given string and '='
     * Parameters
     * ----------
     * parname: string
     ...
     *==========================================================================*/
    template <class T>  //set template name
    int get_param<bool>( std::string parname, bool *value );
    /*==========================================================================
     * Template function
     ...
     *==========================================================================*/
    template <class T>  //set template name
    int get_param( std::string parname, int n, std::vector<T> &values);
};
/*==========================================================================
 * END OF THE CLASS
 *==========================================================================*/
 ...
 *==========================================================================*/
template <class T>  //set template name
int ParseIni::get_param( std::string parname, T *value ){
 ...
 * ------
 * value: bool
 *   if the parameter after '=' is T,t,True,true returns true
 *   if the parameter after '=' is F,f,False,false returns false
 * return
 * ------
 * error: int
 *   0: if no error occurs; 1: if the parameter do not exists, 
 *   -1: if the parameter has no argument; -2 if the parameter is not 
 *   among the choises of above
 *==========================================================================*/
template <class T>  //set template name
int get_param<bool>( std::string parname, bool *value ){
  std::string strbool;   //string that will contain the boolean
  int err = get_param<std::string>(parname, &strbool) //read the parameter
 ...
 *==========================================================================*/
template <class T>  //set template name
int ParseIni::get_param( std::string parname, int n, std::vector<T> &values){
  int error=1;   //error code: by default parname not found
  for(size_t i=0; i<this->inisize; ++i){
    if(this->inis[i].compare(0,parname.size(), parname) == 0){  //if a parameter found
      //get everything after the '='
	...
	std::vector<T> tempvector;  //create a temporary vector
	T tempvar;  //create a temporary variable
...
int ParseIni::get_mcmc_params(std::string parname, double *guess, double* lower,
  ...
  int error = this->get_param<double>( par, 4, paras );
